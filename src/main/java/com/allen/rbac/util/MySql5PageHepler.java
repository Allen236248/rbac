...
public class MySql5PageHepler {
    ...
    public static String getCountString(String querySelect) {
        ...
        int fromIndex = getAfterFromInsertPoint(querySelect);
        ...
        if (select.toLowerCase().indexOf("select distinct") != -1 || querySelect.toLowerCase().indexOf("group by") != -1) {
            ...
            return new StringBuffer(querySelect.length()).append("select count(1) count ").append(querySelect.substring(fromIndex, orderIndex)).toString();
        }
    }
     ...
     */
    private static int getAfterFromInsertPoint(String querySelect) {
